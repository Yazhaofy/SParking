...
-(void)createScrollView{
    ...
    for (int i = 0; i < images.count; i ++) {
        ...
        if (i == images.count - 1) {
            //判断要不要添加button
            if (!isScrollOut) {
                UIButton *enterButton = [[UIButton alloc] initWithFrame:enterBtnFrame];
                if (enterBtnImage) {
                    //点九法填充图片
                    UIImage *img=[UIImage imageNamed:enterBtnImage];
//                UIEdgeInsets edge=UIEdgeInsetsMake(0, 10, 0,10);
//                img= [img resizableImageWithCapInsets:edge resizingMode:UIImageResizingModeStretch];
                    
                    [enterButton setImage:img forState:UIControlStateNormal];
                }else{
